...
def log_normal(x: pd.Series, s: float, mean: float, stddev: float) -> pd.Series:
    return lognorm.pdf(x, s, loc=mean, scale=stddev)
...
def exp_norm(x: pd.Series, mean: float, stddev: float) -> pd.Series:
    return exponnorm.pdf(x, 1.0, loc=mean, scale=stddev)
