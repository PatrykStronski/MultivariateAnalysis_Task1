...
df_sampled = df.loc[df['fire_size_class'] == 'D']

mean, median, var, skewness = get_moments(df_sampled, property)
stddev = math.sqrt(var)
print(f'For {property} G we have mean: {mean}; median: {median}; stddev: {stddev}')
print(df_sampled.size)

kernel = gaussian_kde(df_sampled[property])
min_amount, max_amount = df_sampled[property].min(
), df_sampled[property].max()
x = np.linspace(min_amount, max_amount, num=len(df_sampled))
kde_values = kernel(x)
print(min_amount)

sns.displot(data=df_sampled, x=property,
            label=f'Average size for class G', bins=50, stat="probability")
...
sns.displot(data=df_sampled, x=property,
            label=f'Average size for {property} class', bins=50, stat="probability")
print(x)
plt.plot(x, gamma.pdf(x, mle_gm[0], mle_gm[1], mle_gm[2]), label='GAMMA')
plt.plot(x, lognorm.pdf(
    x, mle_ln[0], loc=mle_ln[1], scale=mle_ln[2]), label='lognormal')
plt.legend()
plt.show()
